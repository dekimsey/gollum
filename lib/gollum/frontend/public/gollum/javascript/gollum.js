// Helpers
// Make pathname local to the baseUrl.
var pathname = baseUrl == '' ? window.location.pathname.substr(1)
                             : window.location.pathname.substr(baseUrl.length + 1);
var slashIndex = pathname.lastIndexOf('/');

// Path name without the leading slash.
function page_path(){
  return pathname.substr(0, slashIndex);
}
...
function rel2abs_page_name(path, name){
  // Make sure the given path starts at the root.
  if(name[0] != '/'){
      name = '/' + path + '/' + name;
  }
  var name_parts = name.split('/');
  var newPath = name_parts.splice(0, name_parts.length - 1).join('/');
  var newName = name_parts.splice(-1)[0];
...
$(document).ready(function() {
  $('#delete-link').click( function(e) {
    ...
    if ( ok ) {
      var loc = baseUrl + '/delete/' + pathname;
      window.location = loc;
    }
    ...
    $('#minibutton-rename-page').click(function(e) {
      ...
      $.GollumDialog.init({
        ...
        OK: function( res ) {
          ...
          var name_parts = rel2abs_page_name(path, newName);
  ...
  if ($('#minibutton-new-page').length) {
    ...
    $('#minibutton-new-page').click(function(e) {
      ...
      var path = page_path();
      // Are we in the page viewer? If os, remove it.
      ...
      $.GollumDialog.init({
        ...
        OK: function( res ) {
          ...
          var name_encoded = [];
          var name_parts = rel2abs_page_name(path, name).join('/').split('/');
